...
## Complete Setup Guide — Products, Policies, Mock Backend & Deployment

**Purpose:** Configure Azure API Management as an AI Gateway in front of Claude deployed on Microsoft Foundry, with tiered token quota enforcement, full observability, synthetic Anthropic-compatible rate limit headers, and a verified local mock backend workflow for policy validation without a live Foundry Claude deployment.

**Audience:** Coding agent / Azure engineer deploying from scratch.
...
| Azure Managed Identity | System-assigned on APIM | Required so APIM can authenticate to the customer's Foundry resource in live deployments |
...
└─────────────────────────────────────────────────────┘
        │
        ▼
  Customer's Microsoft Foundry → Claude
...
3. Click **Save**
4. Note the **Object (principal) ID** — you will need this to grant APIM access to the customer's Foundry resource in live deployments
...
For a live deployment, the backend URL should be the customer's own Claude-capable Azure AI Foundry endpoint. The temporary placeholder described below is only for the separate mock-validation workflow later in this guide.
...
3. Fill in:
   ...
   - **Web service URL:** `https://<your-foundry-resource>.services.ai.azure.com/anthropic`
     > Replace `<your-foundry-resource>` with the customer's own Foundry resource name for live deployments. Only use a placeholder such as `https://mock.placeholder.invalid/anthropic` if you are intentionally following the mock-validation workflow in Step 7.
...
2. Fill in:
  - **Name:** `sample-enterprise-prod` (or `sample-standard-prod`, `sample-restricted-prod`)
   - **Display name:** `Sample Enterprise Subscription`
   - **Scope:** **Product**
...
|---|---|---|
| `sample-enterprise-prod` | claude-enterprise | Sample enterprise validation |
| `sample-standard-prod` | claude-standard | Sample standard validation |
| `sample-restricted-prod` | claude-restricted | Sample restricted validation |
...
4. Under **Subscription**, select one of your sample subscriptions (for example `sample-enterprise-prod`)
5. Click **Send**
...
```powershell
$md = '.\apim-claude-foundry-gateway-setup.md'
$html = '.\apim-claude-foundry-gateway-setup.html'
...
If you use the helper renderer script, run it from the same folder as the Markdown file or pass explicit values for `-MarkdownPath` and `-HtmlPath`.
